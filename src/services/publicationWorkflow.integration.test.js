...
test("Mongo integration: publication limits, snapshot freeze, and immediate Seen publishing", { skip: !testMongoUri }, async () => {
  ...
  try {
    ...
    assert.equal(submitted.status, "PUBLISHED"); assert.equal(submitted.submittedSnapshot.chapters.length, 1); assert.equal(submitted.publishedSnapshot.chapters.length, 1);
