...
const AUTHENTICATED_CONTENT_PREFIX = "onlyme/content/";
...
export function authenticatedDeliveryUrl(media) {
  if (media?.secureUrl && ABSOLUTE_MEDIA_URL.test(media.secureUrl) && !String(media.assetId || "").startsWith(AUTHENTICATED_CONTENT_PREFIX)) {
    return media.secureUrl;
  }
  if (!media?.assetId) return media?.secureUrl || "";
