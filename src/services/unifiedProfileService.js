...
export function serializeUnifiedProfile({ content = [], followerCount = 0, followingCount = 0, owner,  ownWallPosts = [], photos = [], pinnedMessageGroup = null, planets = [], publishedContentCount = content.length, roleProfile, seens = [], sharedSeens = [], sharedWallPosts = [], supporterCount = 0, viewer, viewerRelationships = [] }) {
  ...
  return {
    ...
    sharedSeens: sharedSeens.map((item) => { const publication = serializePublication(item, contentViewer); return publication ? { ...publication, shareId: item.shareId || null, shareCaption: item.shareCaption || "", feedCreatedAt: item.feedCreatedAt || item.createdAt, sharedBy: item.sharedBy || null } : null; }).filter(Boolean),
