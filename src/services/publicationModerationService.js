...
export async function moderatePublication({ adminId, decision, id, review = {} }) {
  ...
  const current = await Publication.findOne({ _id: id, kind: { $in: ["WORLD", "PREMIUM_WORLD"] }, status: "PENDING_REVIEW" }).select("+submittedSnapshot"); if (!current) { if (!await Publication.exists({ _id: id, kind: { $in: ["WORLD", "PREMIUM_WORLD"] } })) throw new ApiError(404, "Planet not found"); throw new ApiError(409, "Planet was already reviewed or changed"); }
  ...
  const publication = await Publication.findOneAndUpdate({ _id: id, kind: { $in: ["WORLD", "PREMIUM_WORLD"] }, status: "PENDING_REVIEW", statusVersion: current.statusVersion, submittedVersion: current.submittedVersion }, { $set: set, $inc: { statusVersion: 1 } }, { new: true }).select("+submittedSnapshot +internalModerationNote");
