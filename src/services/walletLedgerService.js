...
async function walletFor(user, session, { create = false } = {}) {
  ...
  const [wallet] = await Wallet.create([{ user, balance: 0, bonusBalance: 0, currency: "STARS", version: 0, ledgerActivatedAt: new Date(), reconciliationStatus: "MATCHED" }], { session });
  return wallet;
}

async function post({ user, amount, bonusAmount = 0, direction, entryType, entryRole, referenceType, referenceId, publication, creator, counterpartyUser, command, idempotencyKey, reversalOf, metadata }, session) {
  const create = direction === "CREDIT";
  let wallet = await walletFor(user, session, { create });
  if (direction === "DEBIT") {
    const bonusSpent = Math.min(Number(wallet.bonusBalance || 0), amount);
    wallet = await Wallet.findOneAndUpdate(
      { _id: wallet._id, balance: { $gte: amount } },
      { $inc: { balance: -amount, bonusBalance: -bonusSpent, version: 1 }, $set: { currency: "STARS", ledgerActivatedAt: wallet.ledgerActivatedAt || new Date(), reconciliationStatus: "MATCHED" } },
...
export const safeWallet = (wallet) => ({ balance: wallet.balance, bonusBalance: Number(wallet.bonusBalance || 0), version: wallet.version, currency: "STARS" });
