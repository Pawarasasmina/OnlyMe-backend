...
export const listMySeries = asyncHandler(async (req, res) => { const items = await PublicationSeries.find({ creator: req.user._id, archivedAt: null }).sort({ updatedAt: -1, name: 1 }).lean(); return sendResponse(res, 200, "Series fetched", { items: items.map((item) => ({ id: String(item._id), name: item.name, createdAt: item.createdAt, updatedAt: item.updatedAt })) }); });
export const createSeries = asyncHandler(async (req, res) => { const name = normalizeSeriesName(req.body.name); if (!name) throw new ApiError(400, "Series name is required"); const normalizedName = normalizeSeriesKey(name); if (await PublicationSeries.exists({ creator: req.user._id, normalizedName, archivedAt: null })) throw new ApiError(409, "You already have a Series with that name"); try { const series = await PublicationSeries.create({ creator: req.user._id, name, normalizedName }); return sendResponse(res, 201, "Series saved", { series: { id: String(series._id), name: series.name, createdAt: series.createdAt, updatedAt: series.updatedAt } }); } catch (error) { if (error?.code === 11000) throw new ApiError(409, "You already have a Series with that name"); throw error; } });
...
export const submit = asyncHandler(async (req, res) => { await enforcePremiumExperienceCapacity(req.user._id, req.params.id); return sendResponse(res, 200, "Publication submitted", { publication: serializePublication(await submitPublication(req.user._id, req.params.id, req.body), req.user) }); });
export const resubmit = asyncHandler(async (req, res) => { await enforcePremiumExperienceCapacity(req.user._id, req.params.id); return sendResponse(res, 200, "Publication resubmitted", { publication: serializePublication(await resubmitPublication(req.user._id, req.params.id, req.body), req.user) }); });
