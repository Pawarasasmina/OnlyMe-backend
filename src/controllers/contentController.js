...
export const getPublishedContent = asyncHandler(async (req, res) => {
  if (!mongoose.isValidObjectId(req.params.id)) throw new ApiError(400, "Invalid content ID");
  const item = await Content.findOne({ _id: req.params.id, status: { $in: ["PUBLISHED", "published"] } })
    .populate("creator", "name username avatar isVerified")
    .lean();
  if (!item) throw new ApiError(404, "Published content not found");
  return sendResponse(res, 200, "Published content fetched", { content: serializeContent(item, null) });
});
...
export const submitContent = asyncHandler(async (req, res) => {
  const content = await submitDraft(req.user._id, req.params.id);
  await recordChecklistEvent(req.user._id, "createdFirstWorld");
  return sendResponse(res, 200, "Content submitted for review", { content: serializeContent(content, req.user) });
});
