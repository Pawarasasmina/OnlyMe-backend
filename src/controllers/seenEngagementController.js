...
const summary = async (publication, viewerId) => {
  const [counts, viewer, comments] = await Promise.all([
    ...
    viewerId ? SeenEngagement.find({ publication, user: viewerId, type: { $in: ["REACTION", "SHARE"] } }).lean() : [],
  ...
  return { reactionCount: count.REACTION || 0, commentCount: count.COMMENT || 0, shareCount: count.SHARE || 0, viewerReaction: viewer.find((item) => item.type === "REACTION")?.reaction || null, viewerShared: Boolean(viewer.find((item) => item.type === "SHARE")), comments: comments.reverse().map((item) => ({ id: item._id, text: item.text, createdAt: item.createdAt, author: { id: item.user?._id, name: item.user?.name, username: item.user?.username, avatar: item.user?.avatar || "" } })) };
};
...
export const shareSeen = asyncHandler(async (req, res) => { await publishedSeen(req.params.id); await SeenEngagement.updateOne({ publication: req.params.id, user: req.user._id, type: "SHARE" }, { $setOnInsert: { publication: req.params.id, user: req.user._id, type: "SHARE" } }, { upsert: true }); return sendResponse(res, 200, "Seen shared to your profile", { engagement: await summary(req.params.id, req.user._id) }); });
