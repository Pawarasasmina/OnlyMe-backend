...
const notifyCreatorActivity = (req, publication) => {
  if (String(publication.creator) !== String(req.user._id)) {
    req.app.get("io")?.to(`user:${publication.creator}`).emit("activity:updated");
  }
};
...
export const removeSeenReaction = asyncHandler(async (req, res) => { await publishedSeen(req.params.id, req.user, req.body.accessToken || req.query.access || req.query.token); await SeenEngagement.deleteOne({ publication: req.params.id, user: req.user._id, type: "REACTION" }); return sendResponse(res, 200, "Reaction removed", { engagement: await summary(req.params.id, req.user._id) }); });
...
export const reportSeen = asyncHandler(async (req, res) => {
  ...
  const publication = await Publication.findOne({ _id: req.params.id, kind: "SEEN", status: "PUBLISHED" }).select("_id creator kind title summary publishedAt visibility +shareToken").lean();
