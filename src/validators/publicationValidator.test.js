...
const publication = (kind, pricing) => ({ kind, title: "Title", summary: "Summary", category: kind === "SEEN" ? "Lifestyle" : "Category", visibility: "PUBLIC", pricing, coverMedia: { assetId: "verified-cover" } });
...
test("Seen category and visibility use supported production values", () => { assert.doesNotThrow(() => assertCompletePublication({ ...publication("SEEN", { mode: "FREE", starsAmount: null }), category: "Growth", visibility: "FRIENDS" }, [chapter()])); assert.doesNotThrow(() => assertCompletePublication({ ...publication("SEEN", { mode: "FREE", starsAmount: null }), category: "Business", visibility: "LINK_ONLY" }, [chapter()])); assert.throws(() => assertCompletePublication({ ...publication("SEEN", { mode: "FREE", starsAmount: null }), category: "Anything" }, [chapter()])); assert.throws(() => assertCompletePublication({ ...publication("SEEN", { mode: "FREE", starsAmount: null }), visibility: "PRIVATE" }, [chapter()])); });
